...
const ServicesPerformed = async ({ userId }: ServicesPerformedProps) => {
    ...
    const filtered = treatments.filter(t => t.operation && t.operation.userId === userId);

    if (!filtered.length) {
        return <Card className="w-full h-full text-sm text-muted-foreground text-center">Nenhum atendimento realizado.</Card>;
    }

    // Mapear para ServicePerformedTableRow
    const tableData: ServicePerformedTableRow[] = filtered.map(treatment => ({
        id: treatment.id,
        status: treatment.status,
        sectorName: treatment.operation?.servicePoint?.sector?.name || treatment.operation?.servicePoint?.sectorId || "-",
        sectorId: treatment.operation?.servicePoint?.sectorId || "-",
        servicePointName: treatment.operation?.servicePoint?.name || treatment.operation?.servicePointId || "-",
